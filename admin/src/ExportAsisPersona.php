...
switch ($formato) {
    case 'pdf':
        ...
        $pdf->ezText("Reporte de asistencia por persona", 12, array('left' => 305));
        ...
        $pdf->ezSetY(515);
        $pdf->ezText("Asistencia de ".$_SESSION['datos_per'][0]['usuario']." al evento : " . $titulo_evento, 10, array('left' => 0));
        ...
        $pdf->ezSetDy(10);
        // Datos de la tabla
        $datos = $_SESSION['presentaciones'];
        // Títulos para la tabla
        $titulos = array('titulo' => '<b>Título de la presentación</b>', 'finicio' => '<b>Fecha y Hora de Inicio</b>', 'ffin' => '<b>Fecha y Hora de Finalización</b>', 'asistio' => '<b>Asistió</b>');
    ...
    case 'xls':
        ...
        $hoja_trabajo->write(0, 0, 'Título de la presentación', $formato_titulo);
        $hoja_trabajo->write(0, 1, 'Fecha y hora de entrada', $formato_titulo);
        $hoja_trabajo->write(0, 2, 'Fecha y Hora de salida', $formato_titulo);
        $hoja_trabajo->write(0, 3, 'Asistió', $formato_titulo);

        // Agregando los datos a la hoja de trabajo
        $datos = $_SESSION['presentaciones'];
        $fila = 1;
        foreach ($datos as $asistente) {
            $hoja_trabajo->writeString($fila, 0, $asistente['titulo']);
            $hoja_trabajo->writeString($fila, 1, $asistente['finicio']);
            $hoja_trabajo->writeString($fila, 2, $asistente['ffin']);
            $hoja_trabajo->writeString($fila, 3, $asistente['asistio']);
            $fila++;
        }
